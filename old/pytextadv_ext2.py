...
def computeuserinput(command, commands, directions, item, x, y, nosee, nounderstand, help):
    if command in directions:
        x, y = move(command, world, x, y)
        printlocation(world[x][y])
        printroomitems(world[x][y])
    elif command == "look":
        if command == "look":
            printlocation(world[x][y])
            printroomitems(world[x][y])
        elif item in world[x][y].items:
            print world[x][y].items[userinput[5:]]
        elif item in player.inventory:
            print player.inventory[userinput[5:]]
        else:
            print nosee
    elif command == "get":
        get(player.inventory, item, tuple, world[x][y])
    elif command == "i":
...
while True:
    ...
    computeuserinput(command, commands, directions, item, x, y, nosee, nounderstand, help)
